...
```
phcp_impute(haps, ancient_tped, genetic_map, chr, choose_donors = 200, variants)
```
...
* chr: integer. the name of chromosome to impute

* genotyped: logical. Does the output should include a data frame with imputation results for all SNPs that were genotyped in the pseudo-haplotype target? (defaults: FALSE).
...
The file should have the following columns with the same headers (other columns will be ignored): \
...
## Output
The output is a list of data frames with the posterior probabilities of the genotypes in the imputed SNPs. \
