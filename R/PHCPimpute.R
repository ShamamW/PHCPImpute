...
#' @param freq the maximal minor allele frequency in SNPs to consider when choosing the donors
...
#' @param genotyped logical. If TRUE, The output will include the imputation of the SNPs that were genotyped in the pseudo-haplotype data.
...
#' @param freq_donors the maximal minor allele frequency in SNPs to consider when choosing the donors
#' @param variants a data frame with variants to impute that were not genotyped in the pseudo-haplotype data. If the arguments is missing, only SNPs that were genotyped in the pseudo-haplotype will be impute.
#' @return A list of data frames with the posterior probabilities of the possible genotypes in the imputed SNP.
#' \itemize{
#' \item genotyped_variants - imputation results of SNPs that were genotyped in the pseudo-haplotype data. Depriciated if genotyped=F.
#' \item  chosen_variants - imputation results of SNPs that were listed in variants argument.
#' }
#' @export
#' @example
...
phcp_impute = function(haps, ancient_tped, genetic_map, chr, genotyped = F, choose_donors = F, freq_donors = 0.5, variants) {
  colnames(haps)[1:5] = c("chr","id","bp","a0","a1")
  ...
  if(genotyped){
    output = data.frame(as.data.frame(data)[informative_snps,c("chr","bp","a0","a1")], genotype_prob)
  }

  if(exists("variants")){
    mutations_info = haps %>% inner_join(y = variants, by = c("chr", "bp")) %>%
      filter((Ref == a0 & Alt == a1) | (Ref == a1 & Alt == a0)) %>% arrange(bp)
    mutations_genotype = mutations_info %>% select(starts_with("V"))
    if (choose_donors){mutations_genotype = mutations_genotype %>% select(filtered_input[["donors"]])}
    mutations_genotype = as.matrix(mutations_genotype)
    mutations_output = matrix(nrow = nrow(variants), ncol = 3)
  ...
  if(genotyped && exists("variants")){
    imputation = list(genotyped_variants = output, chosen_variants = mutations_output)
  } else if (genotyped) {
    imputation = list(genotyped_variants = output)
  } else if (exists("variants")){
    imputation = list(chosen_variants = mutations_output)
  }
